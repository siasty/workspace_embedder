...
frappe.ui.form.on('Workspace', {
    refresh: function(frm) {
        ...
        if (frm.doc.name) {
            frm.add_custom_button(__('Manage Page Embeds'), function() {
                show_page_embed_manager(frm);
            }, __('Actions'));

            frm.add_custom_button(__('Add Page Embed'), function() {
...
function show_embed_manager_dialog(frm, current_embeds) {
    ...
    if (current_embeds.length === 0) {
        embed_list_html = '<div class="alert alert-info">No page embeds found in this workspace.</div>';
    } else {
            ...
            },
            {
                ...
                change: function() {
                    if (this.get_value() && !d.get_value('embed_name')) {
                        ...
                        const embedName = pageName.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) + ' Embed';
