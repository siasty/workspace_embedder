...
frappe.ui.form.on('Page Embed', {
    refresh: function(frm) {
        ...
        frm.add_custom_button(__('Preview Embed'), function() {
            preview_embed(frm);
        }, __('Actions'));

        frm.add_custom_button(__('Test Page Access'), function() {
            test_page_access(frm);
        }, __('Actions'));

        frm.add_custom_button(__('Add to Workspace'), function() {
            add_to_workspace(frm);
        }, __('Actions'));

        frm.add_custom_button(__('Generate CSS Template'), function() {
    ...
    target_page: function(frm) {
        ...
        if (frm.doc.target_page && !frm.doc.embed_name) {
            frm.set_value('embed_name',
                frm.doc.target_page.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) + ' Embed'
            );
        }
...
function generate_css_template(frm) {
    let css_template = `/* Page Embed Custom Styling */
...
function validate_dimensions(frm) {
    if (frm.doc.display_height && (frm.doc.display_height < 100 || frm.doc.display_height > 2000)) {
        frappe.msgprint({
...
function validate_page_access(frm) {
    frappe.db.get_value('Page', frm.doc.target_page, 'title')
        .then(r => {
            if (r.message && r.message.title) {
                frm.dashboard.add_comment(__('Target page found: {0}', [r.message.title]), 'green', true);
            }
        });
}
...
function setup_field_watchers(frm) {
    ...
    frm.fields_dict.target_page.input.addEventListener('blur', function() {
        if (!frm.doc.embed_name && this.value) {
            let embed_name = this.value.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) + ' Embed';
...
function add_help_text(frm) {
    ...
    frm.set_df_property('iframe_sandbox', 'description',
    ...
    frm.set_df_property('custom_css', 'description',
        __('Add custom CSS to style the embedded page container. Use CSS class: .page-embed-container'));

    // Add description for permissions
    frm.set_df_property('permissions', 'description',
            ...
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message && r.message.success) {
                        frappe.msgprint({
                            message: r.message.message + `<br><br><a href="/app/workspace/${values.workspace}" target="_blank">View Workspace</a>`,
                            title: __('Success'),
                            indicator: 'green'
                        });
                        d.hide();
