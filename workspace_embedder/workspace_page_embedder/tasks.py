...
def cleanup_inactive_embeds():
    ...
    inactive_embeds = frappe.db.get_list(
        ...
        filters={
            "enabled": 0,
            "modified": ["<", cutoff_date.isoformat()]
        },
        fields=["name"]
    )
    ...
    for embed in inactive_embeds:
        try:
            ...
            frappe.logger().error(f"Error deleting embed {embed.name}: {str(e)}")
...
def generate_usage_statistics():
    ...
    try:
        ...
        frappe.logger().error(f"Error generating usage statistics: {str(e)}")