...
def get_page_embed_conditions(user: str = None) -> str:
    """
    ...
    user_roles = ",".join(
        f"'{role}'" for role in frappe.get_roles(user)
    )
...
def check_embed_access(embed_name: str, action: str = "read") -> bool:
    ...
    if not has_page_embed_permission(embed_name, action, user):
        frappe.throw(
            _("You don't have permission to {0} this page embed").format(action),
...
def get_user_accessible_embeds(user: str = None) -> list:
    """
